...
LINGX010 KCZN003 若遇到一条SQL语句执行有点慢，要你进行优化，你要从哪些角度考虑？
(AKA 数据库SQL常见的优化手段有哪些)
...
> 索引失效 ：指数据库查询优化器无法使用已创建的索引，导致执行全表扫描或低效的查询计划，严重影响查询性能。

函数操作索引字段
隐式类型转换
最左前缀原则违反（联合索引，漏缺）
范围查询后的字段失效（联合索引，中间不用= 用 > <）
lIKE 通配符位置在左
OR 条件
...
怎么防止SQL注入？你们的SQL语句如何来防止别人进行SQL注入？
## 最佳实践总结
1. 永远使用参数化查询（预编译语句） - 这是最有效的防护措施 （PreparedStatement 避免字符串拼接SQL）
2. 输入验证 - 在应用层进行严格的输入检查
3. 最小权限 - 数据库用户只授予必要的权限 DCL grant
4. 定期审计 - 监控和记录所有数据库操作
5. 代码审查 - 确保所有SQL查询都经过安全审查
6. 使用ORM框架 - 利用框架的内置安全特性
7. 错误处理 - 不要在错误信息中暴露数据库结构
8. 定期更新 - 保持数据库和框架版本最新
通过实施这些多层防护措施，可以有效防止SQL注入攻击，确保应用程序的数据安全。

> 为什么转义特殊字符 能防止注入？
 "转义特殊字符"确实是指"转义用户输入字符串参数中的特殊字符" 。
 - ✅ 转义对象 ：用户输入的字符串参数
- ✅ 转义内容 ：参数中的特殊字符（如单引号、分号等）
- ✅ 转义目的 ：使这些特殊字符失去SQL语法意义
- ✅ 防护效果 ：防止用户输入改变SQL语句结构

SQL注入转义和XSS防护在 防护思路上确实殊途同归。
共同防护模式：转义用户输入中的特殊字符，使其失去语法意义。


KCZN004 项目里有用到关于数据库事务的东西吗  

是的，我在项目中广泛使用了数据库事务，主要通过Spring Boot的声明式事务管理来实现。虽然我没有手工编写BEGIN、COMMIT这样的SQL语句，但我深度使用了@Transactional注解来保证业务操作的ACID特性
...
"使用Spring的声明式事务管理有以下优势："

1. 代码简洁 ：无需手工管理事务的开始、提交、回滚
2. 异常处理 ：运行时异常自动触发回滚
3. 配置灵活 ：可以通过注解参数精确控制事务行为
4. 测试友好 ：在测试环境中可以自动回滚，保持数据库清洁
 6. 进阶话题（如果面试官继续深入）

"如果涉及分布式系统，我也了解分布式事务的解决方案："

- 两阶段提交（2PC） ：使用Seata等框架
- Saga模式 ：长事务拆分 + 补偿机制
- 最终一致性 ：通过消息队列实现

 回答要点总结
1. 承认使用 ：明确表示项目中使用了事务
2. 技术选型 ：说明使用Spring Boot声明式事务而非手工SQL
3. 具体场景 ：举例说明在哪些业务场景下使用
4. 技术深度 ：展示对事务传播机制的理解
5. 实际代码 ：如果可能，展示具体的代码片段
6. 扩展思维 ：提及分布式事务等进阶话题
这样的回答既承认了你的实际经验，又展示了对事务机制的深入理解，同时避免了"没有使用过"的负面印象。