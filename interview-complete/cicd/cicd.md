...
SANO016
你能举一个就是说你比较熟悉的一个云平台上面，然后就是说如果运维这个时候他碰到一些问题，然后具体你是怎么去解决的？能举一个例子吗？

> lab 平安云的例子，k9s aws 的例子。无非看监控（响应时间慢，高占用，OOM、STW）、看日志、看库、看代码
lab: 复盘总结例子。更像是“你印象深刻的bug是什么/你最有难度的一次生产问题排查经历是什么？
lab: 如果在云原生cloudnative环境中，怎么掺杂讲述？过往的例子尽管是微服务，存在上下游，哪怕是K8s部署的分布式应用，但是问题本身或者排查手段涉及“CICD”的话题似乎不多（我这我不知道算不算）

SANO017
如果我现在就想借助这样一个云环境，我要去做一个灰度发布，这个有做过相关的工作吗?
我没做过。

"从技术架构角度，灰度发布本质上是流量管理和风险控制。我会采用以下方案：首先在Kubernetes环境中，通过Ingress Controller配合Service实现流量分割，比如90%流量到稳定版本，10%到新版本；然后利用Istio或者Nginx Plus的权重路由功能，基于用户标签、地理位置或随机策略进行流量分发。"

>lab: 在阿里云真的做一次灰度。
>lab: alex xu 和 software engineer at google 如何讨论灰度？
>lab: 灰度的业务价值？ 灰度和AB-Test的异同？
