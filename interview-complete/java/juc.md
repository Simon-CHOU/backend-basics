...
乐观锁 vs 悲观锁
划分维度：对待并发冲突的“态度”（是否预设冲突会发生）
公平锁 vs 非公平锁
划分维度：线程获取锁的 “顺序”（是否遵循 “先请求先获得”）
可重入锁 vs 非可重入锁
划分维度：同一线程是否可以 “重复获取已持有的锁”
读写锁 / 共享锁 / 排他锁
划分维度：锁是否允许 “多个线程同时持有”（共享性）
...
并发本质
从理论本质上，线程安全的核心是保证共享数据在并发访问时的一致性。具体来说，就是确保多个线程对同一数据的操作序列化后，结果与单线程执行的结果一致。这涉及到三个关键要素：原子性、可见性和有序性。
原子性确保操作不可分割，可见性保证修改立即可见，有序性防止指令重排。

实现机制
实现机制上，Java提供多层次解决方案：JVM层的synchronized内置锁，JUC包的ReentrantLock显式锁，无锁的CAS原子操作。我在实际项目中会根据场景选择：简单互斥用synchronized，复杂控制用ReentrantLock，高频计数用LongAdder。

工程实践
优先考虑无共享设计，通过线程局部变量或消息传递来避免共享状态。当必须共享时，我会选择合适的并发容器，如用ConcurrentHashMap替代HashMap，并充分利用其原子复合操作如putIfAbsent来避免竞态条件。
在实际项目中，我还会从性能角度考虑锁的粒度和持有时间，比如在高并发缓存场景中，我会使用分段锁或读写锁来提升吞吐量。同时，我也会关注死锁预防，通过统一的锁顺序和超时机制来保证系统的健壮性。

总结
我认为线程安全的关键在于：理解并发的本质问题，选择合适的同步机制，并在架构层面减少共享状态的复杂度。

> lab: 通过线程局部变量或消息传递来避免共享状态，即为一种“进程内架构”。

ZYWL002
怎么用加锁的方式解决同步问题？

解决同步问题的核心是通过锁机制保证多线程对共享资源的互斥访问。Java提供两种主要方式：
synchronized内置锁：基于对象监视器，支持锁升级优化，适合大多数场景
Lock显式锁：提供更灵活的控制，如可中断、超时等待、公平性选择
实际选择时要考虑：并发量、锁竞争程度、是否需要高级功能。通常优先synchronized，有特殊需求再考虑Lock。

> lab: 枚举业务场景，考虑：并发量、锁竞争程度、高级功能，形成决策树

ZYWL009
synchronized加锁的实现原理有了解过吗？

synchronized的实现基于JVM的内置锁机制，底层依赖对象头中的Mark Word存储锁状态。

它有两种使用形式：修饰方法时，JVM通过方法表中的ACC_SYNCHRONIZED标志实现；修饰代码块时，通过monitorenter和monitorexit指令控制，这两个指令对应Monitor对象的获取与释放。

锁会经历偏向锁、轻量级锁、重量级锁的升级过程。偏向锁避免无竞争时的原子操作；轻量级锁用CAS自旋减少阻塞；只有竞争激烈时才膨胀为重量级锁，依赖操作系统互斥量，会导致线程切换开销。

这种锁升级机制体现了JVM的优化思路：根据竞争程度动态调整，在保证线程安全的同时最大化性能。
...
如果有第二个线程竞争，偏向锁会升级为轻量级锁。此时线程会在栈帧中创建锁记录，用 CAS 尝试将对象头的 Mark Word 替换为指向锁记录的指针，失败则自旋重试，避免直接进入内核态阻塞。
当竞争进一步激烈，比如自旋次数超过阈值或有线程阻塞，就会膨胀为重量级锁。此时依赖操作系统的互斥量实现，线程会进入阻塞状态，虽然开销大，但能保证多线程下的安全。
这种升级机制体现了 JVM 的设计思想：用最小的开销解决不同竞争程度的同步问题，平衡安全性和性能。

> lab: 徒手画出 Mark Word 锁升级的过程
