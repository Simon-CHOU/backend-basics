#!/usr/bin/env bash
# =============================================================================
# Protocol Benchmark — One-Click Runner
# =============================================================================
# Usage:
...
COMPLETE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
...
stop_server() {
  ...
  # Also kill anything on ports 8080/9090
  local p8080 p9090
...
clean_intermediates() {
  ...
  rm -f "$SCRIPT_DIR"/jmeter.log
  rm -f "$SCRIPT_DIR"/results-rest-*.csv
  rm -rf "$SCRIPT_DIR"/temp
  ok "Cleaned raw CSVs, jmeter.log, temp/"
  ok "Kept: server-*.log, jstat.log, results/*-report/"
}
...
run_grpc() {
  ...
  # Extract JSON stats and write to .stats file
  python -c "
import json, sys
...
# header
print(f'  {\"Metric\":<22} {\"REST\":>15} {\"GraphQL\":>15} {\"gRPC\":>15}')
print(f'  {\"-\"*22} {\"-\"*15} {\"-\"*15} {\"-\"*15}')
...
" 2>&1
  ...
  echo ""
  echo "  JMeter HTML reports:"
  echo "    REST:    $RESULTS_DIR/rest-report/index.html"
  echo "    GraphQL: $RESULTS_DIR/graphql-report/index.html"
  echo ""
  echo "  Server logs:"
  echo "    stdout:  $SCRIPT_DIR/server-out.log"
  echo "    stderr:  $SCRIPT_DIR/server-err.log"
  echo ""
}
